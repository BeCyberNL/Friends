...
window.addEventListener("onWidgetLoad",function(o){var u=o.detail.fieldData.logoUrl;document.querySelectorAll(".bh-logo").forEach(function(i){if(u)i.src=u;else i.style.display="none";});});
function bhStart(mins){var el=document.querySelector(".bh-countdown");var s=mins*60;setInterval(function(){s=s<=0?mins*60:s-1;el.textContent=("0"+Math.floor(s/60)).slice(-2)+":"+("0"+s%60).slice(-2);},1000);}
var bhBooted=false;window.addEventListener("onWidgetLoad",function(o){bhBooted=true;bhStart(Number(o.detail.fieldData.countdownMinutes)||5);});setTimeout(function(){if(!bhBooted)bhStart(5);},1500);

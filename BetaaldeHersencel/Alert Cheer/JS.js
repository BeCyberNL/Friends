...
window.addEventListener("onEventReceived",function(o){var l=o.detail.listener,ev=o.detail.event||{};if(l!=="cheer-latest")return;if(ev.name===undefined&&ev.data)ev=ev.data;try{q(".bh-name").textContent=ev.name;q(".bh-label").textContent="CHEER ×"+(ev.amount||0);if(ev.message)q(".bh-msg").textContent=String.fromCharCode(34)+ev.message+String.fromCharCode(34);}catch(e){}var r=q(".bh-alert");r.classList.add("bh-show");clearTimeout(bhT);bhT=setTimeout(function(){r.classList.remove("bh-show");},bhDur*1000);});

...
window.addEventListener("onEventReceived",function(o){var l=o.detail.listener,ev=o.detail.event||{};if(l!=="subscriber-latest")return;if(ev.name===undefined&&ev.data)ev=ev.data;try{q(".bh-name").textContent=ev.name+(ev.amount>1?" ×"+ev.amount:"");q(".bh-msg").textContent=(ev.amount||1)+" month"+(ev.amount>1?"s":"")+" of brain rental";}catch(e){}var r=q(".bh-alert");r.classList.add("bh-show");clearTimeout(bhT);bhT=setTimeout(function(){r.classList.remove("bh-show");},bhDur*1000);});
